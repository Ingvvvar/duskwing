...
/**
 * Полоса у земли, отданная переднему плану.
 *
 * Просвет в неё не опускается — это проверяется `tests/foreground-band.test.ts`
 * для каждого конфига уровня, поэтому передний план физически не может
 * перекрыть проход. Число живёт здесь одно на всех: и логика проверки, и
 * рендер берут его отсюда.
 */
export const FOREGROUND_BAND_HEIGHT = 72;
