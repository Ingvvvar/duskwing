...
describe('единственность источника координат зон', () => {
   ...
   *
   * В подходе Б рендер по этому же отображению рисует полосы ветра. Если
   * кто-нибудь заведёт для полос вторую формулу — другой период, другой
   * отсчёт, свой аккумулятор расстояния, — полосы разъедутся с физикой.
